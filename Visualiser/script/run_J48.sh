...
if [ "$VERB_LEVEL" -lt "3" ] ; then
	java -cp ~/veridian/weka-3-8-1/weka.jar weka.classifiers.meta.FilteredClassifier -k -t $DLC_NEWSFRAME_HOME/$CSV_OUTPUT -F "weka.filters.MultiFilter -F \"weka.filters.supervised.instance.SMOTE -C 1 -K 5 -P 100.0 -S 1\" -F \"weka.filters.unsupervised.instance.Randomize -S 42\"" -W weka.classifiers.trees.J48 -- -C 0.25 -M 2
else
